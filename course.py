...
if __name__ == "__main__":
	...
	# section6.mapFunc()
	# section6.filterFunc()
	# from section6 import laLaLambda
	# laLaLambda()
	import math
	assert section6.assignment_sphere(1) == 4*math.pi/3
	assert section6.assignment_sphere(0) == 0
	assert abs(section6.assignment_sphere(2) - 33.510) < 0.001
	
	assert section6.ran_check(5,2,7)
	assert not section6.ran_check(10,2,7)
	
	# print(section6.up_low('Hello Mr. Rogers, how are you this fine Tuesday?'))
	assert (section6.up_low('Hello Mr. Rogers, how are you this fine Tuesday?')) == (4, 33)
	
	assert section6.unique_list([1,1,1,1,2,2,3,3,3,3,4,5]) == [1, 2, 3, 4, 5]
	
	assert section6.multiply([1,2,3,-4]) == -24
	
	assert section6.palindrome('helleh')
	assert section6.palindrome('madam')
	assert section6.palindrome('nurses run')
	
	assert section6.ispangram("The quick brown fox jumps over the lazy dog") == True